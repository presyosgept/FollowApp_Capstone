...
class CompositeForeignKey(ForeignObject):
                ...
                if dependents:
                    ...
                    res.append(
                        checks.Error(
                            ...
                            (self.name, ",".join(
                                f.name for f in dependents), self.name),
                            hint=None,
                            obj=self,
                            id='compositefk.E006',
                        ))
        ...
        for local_field in self._raw_fields.values():
            if isinstance(local_field, LocalFieldValue):
                try:
                    ...
                    res.append(
                        checks.Error(
                            "the field %s does not exists on the model %s" % (
                                local_field, self.model),
                            hint=None,
                            obj=self,
                            id='compositefk.E003',
                        )
    ...
    def deconstruct(self):
        name, path, args, kwargs = super(
            CompositeForeignKey, self).deconstruct()
    ...
    def get_extra_restriction(self, where_class, alias, related_alias):
        ...
        for remote, local in self._raw_fields.items():
            lookup = local.get_lookup(
                self, self.related_model._meta.get_field(remote), alias)
            if lookup:
    ...
    def get_instance_value_for_fields(self, instance, fields):
        ...
        res = super(CompositeForeignKey, self).get_instance_value_for_fields(
            instance, fields)
...
class CompositeOneToOneField(CompositeForeignKey):
    ...
    def deconstruct(self):
        name, path, args, kwargs = super(
            CompositeOneToOneField, self).deconstruct()
        if "unique" in kwargs:
            del kwargs['unique']
        return name, path, args, kwargs
